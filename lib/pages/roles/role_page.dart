...
class RoleManagementWidgetState extends State<RoleManagementWidget> {
  ...
  //     // Construir datos básicos del empleado
  //     final employeeData = {
  //       'Nombre': _nombresController.text,
  //       'Apellidos': _apellidosController.text,
  //       'email': _emailController.text,
  //       'Telefono': _telefonoController.text,
  //       'Role': selectedRole,
  //       'Codigo': _codigoController.text, // Asegurando que 'Codigo' esté presente
  //       'updatedAt': Timestamp.now(),
  //     };
  ...
  //       // Crear un nuevo empleado
  //       final nextCode = await _getNextUserCode();
  //       employeeData['Codigo'] = nextCode; // Sobrescribe el código generado automáticamente
  //       employeeData['createdAt'] = Timestamp.now();
  ...
  //     await _updateUserTeam(selectedUsers);
  ...
  //   final supervisorCode = _codigoController.text; // Código del supervisor
  ...
  //       'Asignado': true, // Marcamos como asignado
  //       'CodSupervisor': supervisorCode, // Agregamos el código del supervisor
  //     });
  //   }
  // }
...
//         final nextCode = await _getNextUserCode();
//         employeeData['Codigo'] =
//             nextCode; // Sobrescribe el código generado automáticamente
//         employeeData['createdAt'] = Timestamp.now();
...
//       // Guardar la lista de selectedUsers en 'MyTeam' del existingEmployee
//       final selectedUsersData =
//           selectedUsers.map((user) => user.toMap()).toList();
//       await userDocRef.update({'MyTeam': selectedUsersData});
...
//     // Encontramos el usuario correspondiente en la lista local y actualizamos su CodSupervisor
//     final user = selectedUsers.firstWhere((u) => u.codigo == doc['Codigo']);
//     user.codigoSupervisor = supervisorCode; // Actualizamos la propiedad en el objeto local
//   }
// }

  /******************************************************************** */

  //ESTE METODO FUNCIONA PERO NO TIENE LOS CAMBIOS DE ACTIVAR DESACTIVAR EL CHECKBOX
  Future<void> _saveOrUpdateEmployee(
      Usuario? existingEmployee, List<Usuario> selectedUsers) async {
    try {
      // Validar datos
      ...
      final employeeData = {
        ...
        'Role': selectedRole,
        'Codigo':
            _codigoController.text, // Asegurando que 'Codigo' esté presente
        'updatedAt': Timestamp.now(),
      };
      ...
      // Eliminar duplicados en la lista 'selectedUsers'
      final uniqueUsers = _removeDuplicateUsers(selectedUsers);

      // Guardar la lista de usuarios únicos en 'MyTeam'
  ...
  //     final usuariosRef = FirebaseFirestore.instance.collection('Users');

  //     // Crear un mapa para almacenar los datos del empleado
  ...
  //       'Codigo': codigo,
  //       'Role': role,
  //       'MyTeam': selectedUsers.map((user) => user.toMap()).toList(),
  //     };
  ...
  //     if (existingEmployee == null) {
  //       // Crear un nuevo empleado
  //       await usuariosRef.add(employeeData);
  //     } else {
  ...
  //       // Asignar el DocumentReference de ese empleado
  //       userDocRef = docRef;
  //     }

  //     // Actualizar el equipo de usuarios en Firestore si el rol es Supervisor
  //     if (role == 'Supervisor' && userDocRef != null) {
  //       // Guardar la lista de usuarios únicos en 'MyTeam'
  //       final uniqueUsers = _removeDuplicateUsers(selectedUsers);
  //       final uniqueUsersData = uniqueUsers.map((user) => user.toMap()).toList();
  //       await userDocRef.update({'MyTeam': uniqueUsersData});
  ...
  Future<void> _updateUserTeam(List<Usuario> selectedUsers) async {
    ...
    try {
      ...
      for (final doc in userQuerySnapshot.docs) {
        ...
        // Encontramos el usuario correspondiente en la lista local y actualizamos sus propiedades
        final user = selectedUsers.firstWhere((u) => u.codigo == doc['Codigo']);
        user.asignado = true; // Actualizamos la propiedad local 'Asignado'
        user.codigoSupervisor = supervisorCode; // Actualizamos la propiedad local 'CodSupervisor'
      }
    } catch (e) {
      print('Error actualizando el equipo de usuarios: $e');
    }
  }
...
//     // New method to determine initial selection
// bool isInitiallySelected(Usuario vendedor) {
//   // Directly check if the vendedor's code is in the MyTeam list
//   return (employee?.myTeam ?? [])
//     .any((teamMember) => teamMember.codigo == vendedor.codigo);
// }
...
//   }
  ...
  //                             onPressed: () {
  //                               if (formKey.currentState!.validate()) {
  //                                 // Verificar si existe un empleado actual para actualizar
  //                                 final existingEmployee =
  //                                     employee; // Asegúrate de que esta variable se defina correctamente

  //                                 // Pasar la lista de usuarios seleccionados
  //                                 final selectedUsers = selectedVendedoresNotifier
  //                                     .value; // Aquí debes asegurarte de que selectedVendedoresNotifier esté correctamente definido

  //                                 _saveOrUpdateEmployee(
  //                                     existingEmployee, selectedUsers);
                                        ...
                                        child: filteredVendedores.isEmpty
                                            ...
                                            : ListView.builder(
                                                ...
                                                itemBuilder: (context, index) {
                                                  final vendedor =
                                                      filteredVendedores[index];
                                                  // Verificar si el vendedor está en el equipo
                                                  bool isSelected =
  ...
  //     final currentSelected = selectedVendedoresNotifier.value;
  //     if (currentSelected.contains(vendedor)) {
  //       // Remove the vendedor if already selected
  //       selectedVendedoresNotifier.value =
  //           currentSelected.where((v) => v.codigo != vendedor.codigo).toList();
  //     } else {
  //       // Add the vendedor if not selected
  //       selectedVendedoresNotifier.value = List<Usuario>.from(currentSelected)
  //         ..add(vendedor);
  //     }
  //   }
  ...
  //     final codigo = vendedor.codigo.toLowerCase();
  //     final trimmedQuery =
  //         query.toLowerCase().trim(); // Evitamos modificar el parámetro
  //     return fullName.contains(trimmedQuery) || codigo.contains(trimmedQuery);
  //   }
